  ...
  try {
    ...
    if (user.role !== "DOCTOR")
      return NextResponse.json(
        {
          success: false,
          message: "Only doctors can acknowledge imaging results",
        },
        { status: 403 },
      );
    ...
    if (study.acknowledgedAt)
      return NextResponse.json({
        success: true,
        message: "Imaging result was already acknowledged",
        data: { acknowledgedAt: study.acknowledgedAt },
      });
    ...
    return NextResponse.json({
      success: true,
      message: "Imaging result acknowledged",
      data: { study: updated },
    });
  } catch (error) {
