...
const prescriptionSelect = { id: true, medicationName: true, dosage: true, frequency: true, route: true, duration: true, durationUnit: true, quantity: true, refills: true, notes: true, reason: true, status: true, startDate: true, endDate: true, createdAt: true, prescriber: { select: { id: true, firstName: true, lastName: true, staffId: true } } };
...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const where = { encounterId: encounter.id, encounter: { hospitalId: user.hospitalId } };
    const [items, total] = await Promise.all([
      prisma.prescription.findMany({ where, orderBy: { createdAt: "desc" }, skip: (page - 1) * limit, take: limit, select: prescriptionSelect }),
      prisma.prescription.count({ where }),
    ]);
    return NextResponse.json({ success: true, data: { items, message: total ? undefined : "No prescriptions found for this encounter", pagination: { page, limit, total, totalPages: total ? Math.ceil(total / limit) : 0 } } });
  } catch (error) {
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const encounter = await findEncounter(id, user.hospitalId);
    if (!encounter) return NextResponse.json({ success: false, message: "Encounter not found" }, { status: 404 });
    if (!canManageEncounter(user, encounter)) return NextResponse.json({ success: false, message: "You are not authorized to prescribe medication" }, { status: 403 });
    ...
    const prescription = await prisma.prescription.create({ data: { patientId: encounter.patientId, encounterId: encounter.id, prescribedBy: user.id, startDate: new Date(), ...result.data }, select: prescriptionSelect });
    return NextResponse.json({ success: true, message: "Prescription submitted successfully", data: { prescription } }, { status: 201 });
  } catch (error) {
    const authResponse = unauthorizedResponse(error);
    if (authResponse) return authResponse;
