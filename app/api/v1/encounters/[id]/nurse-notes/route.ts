...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    const items = await prisma.nurseNote.findMany({
      where: { encounterId: id, hospitalId: user.hospitalId },
      orderBy: { createdAt: "desc" },
      select: nurseNoteSelect,
    });
    return NextResponse.json({
      success: true,
      data: {
        ...
        message: items.length
          ? undefined
          : "No nurse notes found for this encounter",
      },
    });
  } catch (error) {
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    const user = await requireUser();
    if (!canWriteNurseNote(user))
      return NextResponse.json(
        { success: false, message: "Only nurses can create nurse notes" },
        { status: 403 },
      );
    const { id } = await context.params;
    const encounter = await prisma.encounter.findFirst({
      where: { id, hospitalId: user.hospitalId },
      select: { id: true, patientId: true, status: true, lockedAt: true },
    });
    if (!encounter)
      return NextResponse.json(
        { success: false, message: "Encounter not found" },
    ...
    if (!result.success)
      return NextResponse.json(
        {
          success: false,
          message: "Invalid nurse note data",
          errors: result.error.flatten().fieldErrors,
        },
        { status: 400 },
      );
    const nurseNote = await prisma.nurseNote.create({
      data: {
