  ...
  try {
    ...
    if (!result.success)
      return NextResponse.json(
        {
          success: false,
          message: "Explicit confirmation is required to lock this encounter",
    ...
    if (!canManageEncounter(user, encounter))
      return NextResponse.json(
        {
          success: false,
          message: "You are not authorized to lock this encounter",
        },
        { status: 403 },
      );
    if (encounter.status !== "OPEN" || encounter.lockedAt)
      return NextResponse.json(
        { success: false, message: "This encounter is already locked." },
        { status: 409 },
      );
    ...
    return NextResponse.json({
      success: true,
      message: "Encounter note locked successfully",
      data: { encounter: locked },
    });
  } catch (error) {
