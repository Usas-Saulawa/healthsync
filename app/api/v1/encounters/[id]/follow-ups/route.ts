...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    const items = await prisma.followUp.findMany({
      where: { encounterId: encounter.id, hospitalId: user.hospitalId },
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        message: items.length
          ? undefined
          : "No follow-ups found for this encounter",
      },
    });
  } catch (error) {
  ...
  try {
    ...
    if (!canManageEncounter(user, encounter))
      return NextResponse.json(
        {
          success: false,
          message: "You are not authorized to request follow-ups",
        },
        { status: 403 },
      );
    ...
    if (!provider)
      return NextResponse.json(
        {
          success: false,
          message: "Provider is not valid for the selected department",
        },
        { status: 400 },
      );
    const followUp = await prisma.followUp.create({
      data: {
        ...
        scheduledAt: new Date(result.data.preferredDate),
