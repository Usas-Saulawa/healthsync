  ...
  try {
    ...
    if (user.role !== "DOCTOR")
      return NextResponse.json(
        {
          success: false,
          message: "Only doctors can acknowledge laboratory results",
        },
        { status: 403 },
      );
    ...
    if (result.acknowledgedAt)
      return NextResponse.json({
        success: true,
        message: "Laboratory result was already acknowledged",
        data: { acknowledgedAt: result.acknowledgedAt },
      });
    ...
    return NextResponse.json({
      success: true,
      message: "Laboratory result acknowledged",
      data: { result: updated },
    });
  } catch (error) {
