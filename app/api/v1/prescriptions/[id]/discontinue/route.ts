  ...
  try {
    ...
    if (user.role !== "DOCTOR")
      return NextResponse.json(
        {
          success: false,
          message: "Only doctors can discontinue prescriptions",
        },
        { status: 403 },
      );
    const { id } = await context.params;
    const prescription = await prisma.prescription.findFirst({
      where: { id, patient: { hospitalId: user.hospitalId } },
    ...
    if (prescription.status !== "ACTIVE" && prescription.status !== "PAUSED")
      return NextResponse.json(
        {
          success: false,
          message: "Only active or paused prescriptions can be discontinued",
        },
        { status: 409 },
      );
    const result = prescriptionDiscontinueSchema.safeParse(
      await request.json(),
    );
    if (!result.success)
      return NextResponse.json(
    ...
    const updated = await prisma.prescription.update({
      ...
      data: {
        ...
        notes: result.data.reason
          ? `${prescription.notes ? `${prescription.notes}\n` : ""}Discontinued: ${result.data.reason}`
          : prescription.notes,
      },
      select: {
        id: true,
