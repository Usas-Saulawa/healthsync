...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (statusResult && !statusResult.success)
      return NextResponse.json(
        { success: false, message: "Invalid prescription status" },
        { status: 400 },
      );
    ...
    if (dateFrom && dateTo && dateFrom > dateTo)
      return NextResponse.json(
        { success: false, message: "dateFrom cannot be later than dateTo" },
        { status: 400 },
      );
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        pagination: {
          ...
          limit: limitValue,
          total,
          totalPages: total ? Math.ceil(total / limitValue) : 0,
        },
        filters: {
          search: search || null,
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (user.role !== "DOCTOR")
      return NextResponse.json(
        { success: false, message: "Only doctors can prescribe medication" },
        { status: 403 },
      );
    const { id: patientId } = await context.params;
    const patient = await findPatient(patientId, user.hospitalId);
    ...
    if (data.diagnosisId) {
      const diagnosis = await prisma.diagnosis.findFirst({
        where: { id: data.diagnosisId, patientId: patient.id },
        select: { id: true },
      });
      if (!diagnosis)
        return NextResponse.json(
          { success: false, message: "Diagnosis not found for this patient" },
          { status: 400 },
        );
    }
