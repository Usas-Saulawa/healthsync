...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!canCreateEncounter(user))
      return NextResponse.json(
        {
          success: false,
          message: "You are not authorized to create encounters",
        },
        { status: 403 },
      );
...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const typeResult = searchParams.get("type")
      ? z
          .enum(["OUTPATIENT", "INPATIENT", "EMERGENCY", "FOLLOW_UP"])
          .safeParse(searchParams.get("type"))
      : null;
    const statusResult = searchParams.get("status")
      ? z
    ...
    if (
      ...
      (dateFromValue && (!dateFrom || Number.isNaN(dateFrom.getTime()))) ||
      (dateToValue && (!dateTo || Number.isNaN(dateTo.getTime()))) ||
      (dateFrom && dateTo && dateFrom > dateTo)
    ) {
      return NextResponse.json(
        { success: false, message: "Invalid encounter filters" },
        { status: 400 },
      );
    }
    const where = {
      ...
      ...(search
        ? {
            OR: [
              ...
              {
                subjective: { contains: search, mode: "insensitive" as const },
              },
              {
                doctor: {
                  OR: [
    ...
    const history = items.map((item) => ({
      ...
      encounterType: item.type,
      titleOrSubject: item.chiefComplaint || item.clinicalNote || null,
      provider: item.doctor,
      status: item.status,
    }));
    return NextResponse.json({
      success: true,
      data: {
        items: history,
        message: total ? undefined : "No encounters found for this patient",
        pagination: {
          page,
          limit,
          total,
