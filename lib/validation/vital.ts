...
export const createVitalSchema = z.object({
  ...
  if (value.systolicBp !== undefined && value.diastolicBp !== undefined && value.diastolicBp >= value.systolicBp) {
    context.addIssue({ code: "custom", path: ["diastolicBp"], message: "Diastolic blood pressure must be lower than systolic blood pressure" });
  }

  if (Object.keys(value).filter((key) => key !== "encounterId" && key !== "recordedAt").length === 0) {
    context.addIssue({ code: "custom", path: [], message: "At least one vital measurement is required" });
  }
});
