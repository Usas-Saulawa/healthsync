...
export function PatientTable({
  ...
  return (
    <div className="w-full bg-white rounded-3xl border border-blue-100/60 p-6 shadow-xs space-y-4">
      {/* Table Grid Container */}
      <div className="w-full overflow-x-auto">
        <div className="min-w-[1000px] space-y-3">
          {/* Table Header Row */}
          <div className="grid grid-cols-[40px_1.3fr_1fr_0.8fr_1.1fr_1.3fr_1fr_0.9fr] items-center px-5 py-2 text-xs font-bold text-slate-400 uppercase tracking-wider">
          ...
          {patients.map((patient) => {
            ...
            return (
              <div
                key={patient.id}
                className={`grid grid-cols-[40px_1.3fr_1fr_0.8fr_1.1fr_1.3fr_1fr_0.9fr] items-center transition-all px-5 py-3.5 rounded-2xl gap-3 shadow-2xs border ${
                  isSelected
                    ? "bg-blue-50/80 border-blue-200"
                    : "bg-blue-50/40 hover:bg-blue-50 border-transparent hover:border-blue-200/50"
                }`}
              >
                {/* Row Checkbox */}
                <div className="flex items-center">
                ...
                {/* Insurance */}
                <div className="text-xs xl:text-sm text-slate-600 font-medium text-right pr-2 truncate">
                  {patient.insurance}
                </div>
              </div>
            );
